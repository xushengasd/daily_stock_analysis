...
def _append_strategy_synthesis_block(lines: List[str], strategy_synthesis: Any, labels: Dict[str, str], report_language: str) -> None:
    if not isinstance(strategy_synthesis, dict) or not strategy_synthesis:
        return
    ...
    invalid_opinion_count = (
        strategy_synthesis.get("summary_params", {}).get("invalid_opinion_count", 0)
    )
    if invalid_opinion_count and invalid_opinion_count > 0:
    ...
    def generate_dashboard_report(
        ...
        if not self._report_summary_only:
            for result in sorted_results:
                ...
                # ========== 多策略综合 ==========
                strategy_synthesis = dashboard.get('strategy_synthesis', {}) if dashboard else {}
        ...
        if self._report_summary_only:
            ...
            for result in sorted_results:
                ...
                if strategy_synthesis:
                    ...
                    invalid_count = strategy_synthesis.get('summary_params', {}).get('invalid_opinion_count', 0)
