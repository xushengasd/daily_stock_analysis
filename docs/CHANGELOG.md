...
- [修复] 多策略综合类型化投影改为在统一边界逐项过滤非法观点、冲突和主要异议，并在坏 typed candidate 后继续合法 raw fallback；Web/Desktop 为所有非空策略理由提供稳定展开入口，补齐协同推理摘要和中英韩冲突说明；macOS Desktop 构建在生成 DMG 前完成 ad-hoc 签名及严格 bundle 校验。
