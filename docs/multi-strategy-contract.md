...
- `src/agent/orchestrator.py`：在 `_run_skill_stage()` 收尾、`_aggregate_skill_opinions()` 之前，新增 `_partition_skill_opinions(ctx)`——唯一分拣点，把 Valid 保留在 `ctx.opinions`，Invalid 移到 `ctx.meta["invalid_opinions"]`。
- `src/agent/skills/aggregator.py`：`SkillAggregator.aggregate()` 只读 `ctx.opinions`，不再内部 filter；对 `valid_weight_sum == 0` 显式走 `insufficient` 分支。
...
- `src/agent/disagreement.py`：`build_agent_disagreement_summary()` 直接消费 `ctx.opinions`（因分拣已在 Orchestrator 完成），Invalid 完全不出现在 `bullish_agents` / `bearish_agents` / `neutral_agents` 三桶中。
...
- `tests/test_multi_agent.py`：新增 E2E-A..G 反例矩阵，从 SkillAgent 输入 → Orchestrator 分拣 → Aggregator → DecisionAgent prompt → dashboard payload → renderer 实际文本全链路断言。
...
Phase 1 在 `_run_skill_stage()` 收尾后调用 `_partition_skill_opinions(ctx)`：

- 遍历所有 `agent_name` 命中 `is_skill_agent_name()` 的观点。
- 用 `is_valid_strategy_signal(op.signal)` 判定，Invalid 从 `ctx.opinions` 移除并追加到 `ctx.meta["invalid_opinions"]`。
- 分拣完成后进入 `_aggregate_skill_opinions(ctx)`。

Baseline 规定 Orchestrator 是**唯一分拣点**，Aggregator / DecisionAgent / Disagreement 不再重复分拣，直接消费分拣后的 `ctx.opinions`。

### SkillAggregator

只读 `ctx.opinions`，不再内部 filter invalid。计算 `weighted_score` / `weighted_confidence` 严格使用 canonical signal 查 `strategy_signal_score`。对以下三种状态显式走 `insufficient` 分支：
...
产出的 `strategy_synthesis` payload 挂到 `ctx.set_data("skill_consensus", {...})`；Orchestrator 的 `_collect_strategy_synthesis()` 从这里读取，作为 dashboard 的权威合成源。
...
| Orchestrator 分拣点 | `src/agent/orchestrator.py::_partition_skill_opinions`（Phase 1 新增），调用点在 `_run_skill_stage` 收尾、`_aggregate_skill_opinions` 之前 |
| SkillAggregator | `src/agent/skills/aggregator.py::SkillAggregator.aggregate` |
...
| Renderer · Markdown | `src/services/report_renderer.py::_append_strategy_synthesis_block`, `templates/report_markdown.j2` |
...
| 多语言 label 表 | `src/report_language.py::_REPORT_LABELS`, `localize_consensus_level`, `localize_strategy_synthesis_summary`, `labels.none_label` |
| E2E 反例矩阵 | `tests/test_multi_agent.py::TestP1SemanticConvergence` |
