...
class ReportDetails(BaseModel):
    ...
    def populate_strategy_synthesis(cls, value: Any) -> Any:
        ...
        if isinstance(current, StrategySynthesis):
            return populated
        synthesis = extract_strategy_synthesis_payload(current)
        if not synthesis:
            synthesis = extract_strategy_synthesis_payload(populated.get("raw_result"))
        if not synthesis:
            populated["strategy_synthesis"] = None
            return populated
        try:
            populated["strategy_synthesis"] = StrategySynthesis.model_validate(synthesis)
